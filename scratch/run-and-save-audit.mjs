...
async function run() {
  ...
  console.log("⏳ Fetching mobile PageSpeed data from Google API...");
  ...
  writeFileSync("pagespeed-mobile-latest.json", JSON.stringify(data, null, 2));
  console.log("✅ Saved JSON to pagespeed-mobile-latest.json");
  ...
  console.log("\n=== LCP BREAKDOWN ===");
  const lcpItems = audits["lcp-breakdown-insight"]?.details?.items?.[0]?.items ?? [];
  lcpItems.forEach(i => console.log(`  ${i.label}: ${i.duration.toFixed(0)}ms`));
  
  console.log("\n=== UNUSED JS ===");
  const unusedJs = audits["unused-javascript"]?.details?.items ?? [];
  unusedJs.slice(0, 5).forEach(i => {
    console.log(`  -${((i.wastedBytes ?? 0)/1024).toFixed(0)}KB : ${i.url}`);
  });
  
  console.log("\n=== RENDER BLOCKING ===");
  const blocking = audits["render-blocking-resources"]?.details?.items ?? [];
  blocking.forEach(i => {
    console.log(`  ${i.url} (wasted: ${i.wastedMs}ms)`);
  });
}
