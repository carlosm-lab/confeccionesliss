...
**Context:** Se integraron las especificaciones de IA de PageSpeed Insights y navegadores de Inteligencia Artificial (WebMCP y llms.txt) sin alterar ningún componente o lógica existente del carrito de compras.
**Decisions Made:**
...
2. **Protocolo WebMCP (Web Model Context Protocol):**
   - Se implementó `<WebMCPRegistration />` en `src/components/ai/WebMCPRegistration.tsx` e integrado en `src/app/layout.tsx` dentro de `<body>` para evitar conflictos de hidratación SSR/DOM.
   ...
   - Se añadieron anotaciones semánticas WebMCP (`data-webmcp-tool`, `data-webmcp-name`, `data-webmcp-description`) en formularios independientes respetando al 100% los modales y lógica del carrito.
