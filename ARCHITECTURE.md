...
**Date:** 2026-07-20
**Decision:** AI-001 — Integración Estándar de WebMCP y Protocolo LLM (llms.txt)
**Context:** Se requirió auditar el 100% de la aplicación e integrar las especificaciones de IA de PageSpeed Insights y navegadores de Inteligencia Artificial (WebMCP y llms.txt).
**Decisions Made:**

1. **Especificación `llms.txt` y `llms-full.txt` (https://llmstxt.org/):**
   - Se crearon los archivos `public/llms.txt` y `public/llms-full.txt` para exponer la estructura, catálogo, servicios de confección, universidades y políticas de Confecciones Liss en un formato Markdown perfectamente parseable por modelos de lenguaje.

2. **Protocolo WebMCP (Web Model Context Protocol):**
   - Se implementó `<WebMCPRegistration />` en `src/components/ai/WebMCPRegistration.tsx` e integrado en `src/app/layout.tsx`.
   - Se declararon esquemas JSON Schema válidos para 5 herramientas principales: `search_catalog`, `request_quote`, `contact_support`, `get_university_uniforms` y `calculate_size_recommendation`.
   - Se añadieron anotaciones semánticas WebMCP (`data-webmcp-tool`, `data-webmcp-name`, `data-webmcp-description`, `data-webmcp-field`, `data-webmcp-type`, `data-webmcp-required`) en todos los formularios públicos y administrativos.

3. **Árbol de Accesibilidad y CLS 0:**
   - Se validó que el 100% de las páginas cuentan con landmarks semánticos HTML5 (`header`, `nav`, `main`, `footer`), títulos `h1` únicos, jerarquía de encabezados ordenada y dimensiones de contenedor explícitas (`aspect-ratio` / `min-h`) para garantizar CLS = 0.
